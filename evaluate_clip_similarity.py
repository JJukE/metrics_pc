...
def parse_args():
    ...
    parser.add_argument('--batch-size', type=int, default=32, mhelp='Batch size to use')
    ...
    parser.add_argument('--num-workers', type=int, default=None, help='Number of processes to use for data loading. (Default: min(8, num_cpus)')

    parser.add_argument('--real_flag', type=str, default='img', help='The modality of real path')
    parser.add_argument('--fake_flag', type=str, default='txt', help='The modality of fake path.')
    parser.add_argument('--real_path', type=str, required=True, help='Paths to the generated images or to .npz statistic files')
    parser.add_argument('--fake_path', type=str, required=True, help='Paths to the input text or to .npz statistic files')
    parser.add_argument('--num_views', type=int, default=5, help="Number of views to compute CLIP-S"
    )
...
def main(args):
    ...
    if args.num_views > 1:
        clip_s, clip_s_max, clip_s_min = 0.0, 0.0, 1e+5
        for i in range(args.num_views):
            real_path = os.path.join(args.real_path, f"view_{i}")
            dataset = DummyDataset(real_path, args.fake_path,
            ...
            clip_score = calculate_clip_score(dataloader, model,
                                            args.real_flag, args.fake_flag).cpu().item()
            clip_s += clip_score
            clip_s_max = max(clip_s_max, clip_score)
            clip_s_min = min(clip_s_min, clip_score)
        clip_s /= args.num_views
        result = {"avg": float(clip_s), "max": float(clip_s_max), "min": float(clip_s_min)}
    else:
        dataset = DummyDataset(real_path, args.fake_path,
                                args.real_flag, args.fake_flag,
                                transform=preprocess, tokenizer=clip.tokenize)
        dataloader = DataLoader(dataset, args.batch_size, 
                                num_workers=num_workers, pin_memory=True)
        clip_s = calculate_clip_score(dataloader, model,
                                            args.real_flag, args.fake_flag).cpu().item()
        result = {"clip_s": float(clip_s)}
    print('CLIP Score: ', clip_s)
    return result
