# Reference: https://github.com/JRyanShue/NFD/blob/main/nfd/neural_field_diffusion/metrics/render_utils.py
import os

# os.environ['PYOPENGL_PLATFORM'] = 'egl' # use osmesa instaed of egl
import matplotlib
...
class Render:
    ...
    def __init__(self, size, camera_pose, background=None, light=True):
        self.size = size
        global SIZE
        SIZE = size
    ...
    def render(self,
        ...
        if not isinstance(mesh, trimesh.Trimesh):
            raise AttributeError(f"The input mesh is not trimesh.Trimesh, but {type(mesh)}.") # mesh = prepare_mesh(path, color=False, clean=clean)
        try:
...
def pyrender_rendering(mesh,
    ...
    if light:
        color, depth = r.render(scene,
                                flags=pyrender.constants.RenderFlags.ALL_SOLID) # | pyrender.constants.RenderFlags.FACE_NORMALS)
    else:
        color, depth = r.render(scene,
